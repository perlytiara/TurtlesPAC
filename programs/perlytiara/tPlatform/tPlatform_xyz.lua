...
if #tArgs == 3 then -- no error check
  ...
  userZ=tonumber(tArgs[3])
elseif #tArgs == 2 then -- no error check
  blnAskForParameters=false
  userX=tonumber(tArgs[1])
  userY=tonumber(tArgs[2])
end
...
print("|   determine size, and optional z    |")
print("|   height. Either when asked         |")
print("|   by program or with function call, |")
print("|   e.g., tPlatform 5 10 3            |")
...
if blnAskForParameters then
  ...
  -- step 1.3 get z
  write("Enter height z (default&min=1):")
  userZ=read()
  if userZ==nil or userZ=="" then userZ=1 end
  userZ=tonumber(userZ)
  if userZ<1 then userZ=1 end
end  
userX=math.floor(userX)
userY=math.floor(userY)
userZ=math.floor(userZ)

-- check fuel level
local cMinFuel=(userX)*(userY+1)+1
-- account for z layers and vertical travel (up and down between layers)
cMinFuel = userZ * cMinFuel + 2*(userZ-1)
