...
-- Networking helpers (CC:Tweaked rednet)
local EMT_PROTOCOL = "EMT"
local emt_has_modem = false
local emt_monitor_id = nil
local total_tunnels, total_layers_per_tunnel = 0, 0
local current_tunnel, current_layer = 0, 0
local steps_since_status = 0

local function emt_open_modem()
	if peripheral and rednet then
		...
		-- Fallback: open any modem if wireless check not available
		if not emt_has_modem then
...
local function emt_build_inventory()
	local inv = {}
	for slot=1,16 do
		local detail = turtle.getItemDetail(slot)
		if detail then
			inv[#inv+1] = {slot=slot, name=detail.name, count=detail.count}
		end
	end
	return inv
end

local function emt_send(kind, extra)
	if not emt_has_modem then return end
	local msg = {
		kind = kind or "status",
		...
		progress = {
			...
			total_layers = total_layers_per_tunnel,
		},
		pos = emt_get_pos(),
		inventory = emt_build_inventory(),
	}
	if extra then
		for k,v in pairs(extra) do msg[k]=v end
	end
	rednet.broadcast(msg, EMT_PROTOCOL)
	if emt_monitor_id then
		rednet.send(emt_monitor_id, msg, EMT_PROTOCOL)
	end
end

local function emt_poll()
	if not emt_has_modem then return end
	local sender, msg, proto = rednet.receive(EMT_PROTOCOL, 0)
	while sender do
		if type(msg) == "table" and msg.command then
			if msg.command == "request_status" then
				emt_send("status", {requested=true})
			elseif msg.command == "set_monitor" then
				emt_monitor_id = sender
				rednet.send(sender, {kind="ack", action="set_monitor"}, EMT_PROTOCOL)
			elseif msg.command == "request_pickup" then
				emt_send("pickup", {pickup=true})
			end
		end
		sender, msg, proto = rednet.receive(EMT_PROTOCOL, 0)
	end
end
...
function moveForward(FacingAngle, Boolfb, moving_integer, digUpBool, digDownBool, refuel_count)
	...
	for moving_count=1,moving_integer do
		...
		steps_since_status = steps_since_status + 1
		if (steps_since_status % 10 == 0) then emt_send("heartbeat") end
		emt_poll()
	end 
	return refuel_count  
end
...
-- Open wireless modem and announce run start
emt_open_modem()
emt_send("start", {config=true})
textOutput("", 1, 11, 20)
...
total_tunnels = tunnels_integer
total_layers_per_tunnel = math.ceil(Length/2)
current_tunnel, current_layer = 0, 0
refuel_count = moveUp(true, 1, refuel_count, false)
if (Width == 1) then
	refuel_count = moveForward(FacingAngle, true, 1, true, true, refuel_count)
end
for done_tunnels=1, tunnels_integer do
	current_tunnel = done_tunnels
	if (Width >= 2) then
		for done_layers=1, math.ceil(Length/2) do
			current_layer = done_layers
			refuel_count = moveForward(FacingAngle, true, 1, true, true, refuel_count)
	...
	elseif (Width == 1) then
		...
		current_layer = total_layers_per_tunnel
		emt_send("progress")
	end
	if (Height_Position == "up") then
		refuel_count = moveUp(false, (Height - 3), refuel_count, false)
